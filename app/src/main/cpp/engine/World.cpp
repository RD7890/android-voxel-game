...
float World::terrainHeight(int wx, int wz) const {
    float x = wx * 0.04f, z = wz * 0.04f;
    float h = 12.0f;
    h += 10.0f * smoothNoise(x * 1.0f, z * 1.0f);
    h +=  5.0f * smoothNoise(x * 2.1f + 1.7f, z * 2.1f + 0.9f);
    h +=  2.5f * smoothNoise(x * 4.3f + 3.1f, z * 4.3f + 2.3f);
    return h;
}
...
void World::generateChunk(Chunk& chunk) {
    ...
    static constexpr int SEA_LEVEL = 14;
    ...
    for (int z = 0; z < CHUNK_SIZE; z++) {
        ...
        for (int y = 1; y < h - 3; y++) chunk.setBlock(x, y, z, BlockType::STONE);
        for (int y = h-3; y < h; y++) chunk.setBlock(x, y, z, BlockType::DIRT);
